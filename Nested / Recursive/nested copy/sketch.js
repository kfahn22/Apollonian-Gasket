...
const min_nesting_radius = 3; // the smallest radius we will allow
...
function setup() {
  ...
  next_outer_z.push(z1);
  next_outer_z.push(z2);
  period = circles[0].r;
...
function init_ring() {
  ...
  for (i = 0; i < next_outer_z.length; i++) {
    c_outer[i] = new Circle(-next_outer_k / scale_f, next_outer_z[i]);
    console.log(c_outer[i]);
  }
  ...
  for (i = 0; i < c_outer.length; i++) {
    z1[i].push(new Complex(
      c_outer[i].z.x,
      c_outer[i].z.y + c_outer[i].r - 1 / k1
    )); // below
    z2[i].push( new Complex(
      c_outer[i].z.x,
      c_outer[i].z.y - c_outer[i].r + 1 / k2
    )); // above
  
  // use law of cosines to find the third center
  let theta = acos((c * c - b * b - a * a) / (-2 * a * b));
  // smallest circle connects to the right
  z3.push( new Complex(z1[i].x + a * sin(theta), z1[i].y - a * cos(theta)));
  
  // make 3 * c_outer.length circles with kn and zn
  let c1 = new Circle(k1, z1[i]);
  let c2 = new Circle(k2, z2[i]);
  let c3 = new Circle(k3, z3[i]);
    }
  ...
  for (let i = 0; i < z1.length; i++) {
    next_outer_z.push(z1[i]);
  }
  for (let i = 0; i < z2.length; i++) {
    next_outer_z.push(z2[i]);
  }
  for (let i = 0; i < z3.length; i++) {
    next_outer_z.push(z3[i]);
  }
